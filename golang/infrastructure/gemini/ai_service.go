...
func (s *GeminiAIService) GetCompositionAdvice(ctx context.Context, imageBytes []byte, mimeType string) (*model.CompositionAnalysis, error) {
	...
	prompt := `あなたはプロの写真家です。画像を見て構図改善のアドバイスをください。
        ...
        {
            "advice": "60文字以内の具体的なアドバイス（専門用語禁止、命令形ではなく提案）",
            "category": "被写体の種類 ('person', 'food', 'scenery', 'other')", 
            "visual_cues": [
                {
                "target": "camera",
                "direction": "動かす方向 ('left', 'right', 'up', 'down', 'up_left', 'up_right', 'down_left', 'down_right', 'forward', 'backward')"
                }
            ]
        }
        ### 制約事項・判断基準:
        ...
        2. direction は以下から選択してください：
           - "left", "right", "up", "down": 上下左右に平行移動すべき場合
           - "up_left", "up_right", "down_left", "down_right": 斜め方向に平行移動すべき場合
           - "forward": 被写体に寄るべき場合（ズームではなく移動）
           - "backward": 被写体から離れるべき場合（ズームではなく移動）
        3. 複数の指示がある場合は配列に追加してください（例: 右上に移動して、寄る）。
	...
	resp, err := s.Client.GenerativeModel("gemini-2.5-flash").GenerateContent(ctx, content...)
	if err != nil {
